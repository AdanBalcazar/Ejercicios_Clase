...
Con las teclas se interactúa con la animación:
	P: Activa la animación
	O: Detiene la animación
	I: Cambia el sentido de movimiento del coche (Izquierda o Derecha)
...
//Cosas a subir a partir de esta práctica:
// Video .mp4
// Modelos .obj/.mtl
// Texturas .jpg/.png
// Código Fuente
...
//Para la animación del modelo
float movKit_z = 0.0f;
float rotacion_llantas = 0.0f;
...
void animate(void)
{
	...
	if (play)
	{
		if (inversa)  //Avanza hacia atrás (Inversa)
		{
			movKit_z -= 0.03f;
			rotacion_llantas -= 10.0;
		}
		else          //Avanza hacia adelante
		{
			movKit_z += 0.03f;
			rotacion_llantas += 10.0;
		}
...
void display(Shader shader, Model modelo, Model llantas)
{
	...
	shader.setMat4("model", model);
	modelo.Draw(shader);                       //Con esta línea se dibuja 

	// ------- Llantas -------
	model = glm::translate(tmp, glm::vec3(4.6f, -0.5f, 2.2f));
	model = glm::rotate(model, glm::radians(rotacion_llantas), glm::vec3(1.0f, 0.0f, 0.0f));
...
int main()
{
	...
	// -------- La carga del shader ahora se hace aquí en el main() 
    ...
    while (!glfwWindowShouldClose(window))
    {
		...
		display(modelShader, ourModel, llantasModel);  //Aquí se pasan los modelos al display para ir dibujando (Agregar más modelos si es necesario)
